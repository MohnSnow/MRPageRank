...
public class PageRankMapper extends
	...
	private Text outKey = new Text();//Mapper类的输出Key是页面的ID
	
	private PRWritable outValue = new PRWritable();//Mapper的输出Value有两种，一种是页面的邻接链表，一种是页面的当前部分PageRank值

	@Override
	protected void setup(
			Mapper<LongWritable, Text, Text, PRWritable>.Context context)
			throws IOException, InterruptedException {
		super.setup(context);
		context.getCounter(PRCounters.PAGES_COUNT).setValue(0);
		context.getCounter(PRCounters.DANGLING_PAGE_PR).setValue(0);
	}
	...
	protected void map(LongWritable key, Text value, Context context)
		...
		try {
			...
			if(linkListSize==0){
				...
				outKey.set(pageId);
				LLWritable out = new LLWritable(pageId, pageRank, new String[0]);
				outValue.set(out);
				...
				//邻接链表不为空
				String[] ll = new String[linkListSize];
				//为每个目标页面设置PageRank
